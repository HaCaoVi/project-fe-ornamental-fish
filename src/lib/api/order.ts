...
import { COUNT_CART_TAG, LIST_ORDER_OF_USER_TAG, LIST_ORDER_TAG } from "@lib/constants/tag.constant";
...
export const listOrderAPI = async (
    ...
    const params = new URLSearchParams(query).toString()
    const url = `/api/v1/orders/list-order?${params}`
    return sendRequest<IBackendRes<IPagination<IOrder[]>>>(url, {
        method: "GET",
        next: { tags: [LIST_ORDER_TAG], revalidate: 60 },
    },)
}

export const updateStatusOrderAPI = async (orderId: string, status: string) => {
    const url = `/api/v1/orders/update-status/${orderId}`
    const res = await sendRequest<IBackendRes<any>>(url, {
        method: "PATCH",
        body: JSON.stringify({ status })
    })
    if (res.statusCode === 200) {
        revalidateTag(LIST_ORDER_TAG)
    }
    return res;
};